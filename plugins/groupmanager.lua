...
if matches[1] == 'setwelcome' and matches[2] then
	if not lang then
		...
		redis:hset('delete_welcome',msg.chat_id_,tostring(welcome))
...
if matches[1] == 'delwelcome' then
	if not lang then
		if not redis:hget('delete_welcome',msg.chat_id_) then
			tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'Already No welcome message available!', 1, 'md')
		else
			redis:hdel('delete_welcome',msg.chat_id_)
...
local function pre_process(msg)
	if msg.content_.members_ then
		if redis:hget('delete_welcome',msg.chat_id_) then
			if msg.content_.members_[0] then
				name = msg.content_.members_[0].first_name_
				if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
					return nil
				else
					data = redis:hget('delete_welcome',msg.chat_id_)
